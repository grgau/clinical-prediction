...
def testModel():
  ...
  with session as sess:
    for batchIndex in range(nBatches):
      ...
      x_hot, mask_hot, nVisitsOfEachPatient_List = prepareHotVectors(batchX)
      y_hot = np.zeros([1, x_hot.shape[1], x_hot.shape[2]], np.float64) # We need to feed a target tensor to graph for teacher forcing, which is not used in testing (inference) time
      ...
      # make prediction
      predicted_y = sess.run(predictions, feed_dict={x: x_hot, y: y_hot, mask: mask_hot, seqLen: nVisitsOfEachPatient_List})
